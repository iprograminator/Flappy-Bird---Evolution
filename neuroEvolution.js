...
var Neuroevolution = function(options){
	...
	Network.prototype.getSave = function(){
		...
		for (var i in this.layers){
			...
			for (var j in this.layers[i].neurons){
				for (k in this.layers[i].neurons[j].weights){
	...
	Generation.prototype.addGenome = function (genome){
		for(var i = 0; i < this.genomes,length; i++){
			if(self.options.scoreSort < 0){
				if(genomes.score > this.genomes[i].score){
					break;
				}
			} else{
				if (genomes.score < this.genomes[i].score){
		...
		for (var nb = 0; nb < nbChilds; nb ++){
			...
			for (var i in data.network.weights){
				if(Math.random() <= self.options.mutationRate){
					data.network.weights[i] += Math.random()*self.options.mutationRange*
	...
	Generation.prototype.generateNextGeneration = function(){
		...
		while (true){
			for (var i = 0; i < max; i ++){
				var childs = this.breed(this.genomes[i], this.genomes[max],(self.options
					.population.nbChild > 0 ? self.options.nbChild :1));
					for(var c in childs){
	...
	self.nextGeneration = function(){
		...
		if (self.options.lowHistoric){
			if(self.generations.generations.length >=2){
				var genome = self.generations.generations[self.generations.generations.length - 2].genomes;
		...
		if(self.options.historic != -1){
			if(self.generations.generations.length > self.options.historic + 1){
				self.generations.generations.splice(0, self.generations.generations.length - 
					(self.options.historic + 1));
			}
		}
		return nns;
	}
